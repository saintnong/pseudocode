...
public:
    ...
    RuntimeValue call(Interpreter &interpreter, std::vector<RuntimeValue> arguments) override {
        (void)interpreter;
        (void)arguments;
        auto instance = std::make_shared<Instance>(std::static_pointer_cast<Callable>(shared_from_this()));
        RuntimeValue result;
...
RuntimeValue Interpreter::evaluate(Expr *expr) {
    ...
    if (auto *e = dynamic_cast<AssignExpr*>(expr)) return visitAssignExpr(e);
    if (auto *e = dynamic_cast<BinaryExpr*>(expr)) return visitBinaryExpr(e);
    if (auto *e = dynamic_cast<CallExpr*>(expr)) return visitCallExpr(e);
    if (auto *e = dynamic_cast<GetExpr*>(expr)) return visitGetExpr(e);
    if (auto *e = dynamic_cast<ArrayAccessExpr*>(expr)) return visitArrayAccessExpr(e);
    if (auto *e = dynamic_cast<ArrayLitExpr*>(expr)) return visitArrayLitExpr(e);
    if (auto *e = dynamic_cast<NewExpr*>(expr)) return visitNewExpr(e);
 ...
 */
void Interpreter::checkNumberOperands(const Token &operatorToken, 
                                     const RuntimeValue &left, const RuntimeValue &right) {
    if ((left.is<int>() || left.is<double>()) && 
        (right.is<int>() || right.is<double>())) return;
...
RuntimeValue Interpreter::visitAssignExpr(AssignExpr *expr) {
    ...
    if (auto *varExpr = dynamic_cast<VariableExpr*>(expr->target.get())) {
        ...
        if (!arrayVal.is<std::shared_ptr<std::vector<RuntimeValue>>>()) {
            throw RuntimeError(Token{}, "Can only index arrays.");
        }
        if (!indexVal.is<int>()) {
            throw RuntimeError(Token{}, "Array index must be an integer.");
        }
        ...
        if (idx < 0 || idx >= static_cast<int>(arr->size())) {
            throw RuntimeError(Token{}, "Array index out of bounds.");
        }
...
RuntimeValue Interpreter::visitCallExpr(CallExpr *expr) {
    ...
    if (arguments.size() != static_cast<size_t>(function->arity())) {
        throw RuntimeError(Token{}, "Expected " + std::to_string(function->arity()) + 
