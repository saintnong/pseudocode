...
UserFunction::UserFunction(std::shared_ptr<CompiledFunction> compiledFn, std::shared_ptr<Environment> closure,
...
NativeFunction::NativeFunction(int arity, NativeFn function)
...
UserClass::UserClass(const std::string &n, std::shared_ptr<UserClass> s)
    : name(n), superclass(s) {}
...
void UserClass::addField(const std::string &fieldName, std::shared_ptr<CompiledFunction> valueFunc) {
...
std::shared_ptr<UserFunction> UserClass::findConstructor() {
    ...
    if (constructor != nullptr) return constructor;
    if (superclass != nullptr) {
...
int UserClass::arity() {
    ...
    if (constructor != nullptr) return constructor->arity();
    return 0;
}
